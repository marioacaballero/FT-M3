...
async function problemA() {
  ...
  var filenames = [1, 2].map(function (n) {
    return "poem-two/" + "stanza-0" + n + ".txt";
  });

  const res = await Promise.all(
    //para lanzar en paralelo
    filenames.map((archivo) => promisifiedReadFile(archivo))
  );
  for (const poem of res) {
    //crea un objeto iterable para un array o similar
    blue(poem);
  }
...
async function problemC() {
  ...
  for (let i = 0; i < filenames.length; i++) {
    const poem = await promisifiedReadFile(filenames[i]);
    blue(poem);
  }
  console.log("done");
}
